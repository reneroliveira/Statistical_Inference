...
\begin{document}
	...
	\begin{enumerate}
		...
		\begin{teo}
			\textbf{(Teorema de Karlin-Rubin)(citar casella 391-392)} Sejam as hipóteses:
			...
			Seja $T=r(\bX)$ uma estatística suficiente para $\theta$ e que a sua família de distribuições conjunta $\{g(t|\theta)|\theta\in\Omega\}$ tem razão de verossimilhança monótona. Assim, para qualquer $c$, o teste $\delta\pow*$ que rejeita $H_0$ $T\geq c$ é um teste UMP ao nível $\alpha_0$, onde $\alpha_0=\pr[T\geq c\,|\theta=\theta_0]$.
		\end{teo} 
	
	\textbf{Demonstração:} Primeiramente devemos mostrar que o tamanho do teste é $\alpha_0$, ou seja:
	
	$$\sup_{\theta\leq\theta_0}\pr[T\geq c\,|\theta]$$
	
	Se a função poder for não-decrescente, o supremo ocorrerá em $\theta_0$ fica provada essa parte, pois foi suposto que $\pr[T\geq c\,|\theta=\theta_0]=\alpha_0$.
	
	Provaremos então que $\pi(\theta|\delta\pow*)$ é não-decrescente em $\theta$, ou seja, dados $\theta_1<\theta_2$, tem-se $\pi(\theta_1|\delta\pow*)\leq \pi(\theta_2|\delta\pow*)$, que equivale a provar que:
	
	\begin{align*}
		&\pr(T\geq c|\theta_1)\leq \pr(T\geq c|\theta_1)\\\Leftrightarrow&\; 1-G(c|\theta_1)\leq 1-G(c|\theta_2)\\
		\Leftrightarrow&\; G(c|\theta_1)\geq G(c|\theta_2)
	\end{align*}
	
	Onde $G(t|\theta_i),\,i=1,2$ é a distribuição acumulada condicional a $\theta_i$ da estatística $T=r(\bX)$
